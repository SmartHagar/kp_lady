...
const SubMenu: FC<Props> = ({ subMenus, addClass }) => {
  ...
  return (
    <motion.ul
      className={`absolute flex flex-col bg-white shadow-lg rounded-lg z-50 ${
        addClass ? addClass : "-left-4 mt-2"
      }`}
      initial="hidden"
      animate="visible"
      exit="exit"
        ...
        return (
          ...
          >
            <Link
              ...
              } block py-2 px-4 text-color-2 hover:text-primary hover:font-bold transition-colors duration-300`}
            >
              <span>{item.name}</span>
              {item.subMenus && (
                <span className="ml-2">
